...
def generate_bp_circ_taylor(
    ...
    for prune_level in range(1, nsegments):
        ...
        idx_circular_snap = np.where(snap_active_mask)[0]
        idx_taylor = np.where(~snap_active_mask)[0]
        if idx_circular_snap.size > 0:
            dparam_d_vec_new = transforms.shift_taylor_circular_errors(
                dparam_d_vec[idx_circular_snap],
                delta_t,
                p_orb_min,
                use_conservative_tile,
            )
            dparam_cur_batch[idx_circular_snap] = dparam_d_vec_new[:, :-1]
        if idx_taylor.size > 0:
            dparam_d_vec_new = transforms.shift_taylor_errors(
                dparam_d_vec[idx_taylor],
                delta_t,
                use_conservative_tile,
            )
            dparam_cur_batch[idx_taylor] = dparam_d_vec_new[:, :-1]
